...
const SelectScreeningDates = ({ movieName, theater, setIsOrderTicketOpen }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            const requestDetails = getUrlAndSetCallback();
            try {
                const response = await axios.get(serverUrl + requestDetails.url, {
                    cancelToken: newRequest.token,
                })

                requestDetails.callback(response.data);
            } catch (err) {
        ...
        const getUrlAndSetCallback = () => {
            if (selectedMovie && selectedTheater) {
                return { url: `/get-dates?nameInHebrew=${selectedMovie}&theatersName=${selectedTheater}`, callback: setScreeningDates }
            } if (!selectedTheater) {
                return { url: `/get-theaters?nameInHebrew=${selectedMovie}`, callback: setTheaters };
            } else {
                return { url: `/movie/get-movies?theatersName=${selectedTheater}`, callback: setMovies };
            }
        }
        fetchData();
